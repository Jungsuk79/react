import React from 'react';
import { useForm } from 'react-hook-form';
import S from './style';

const Join = () => {

    const {register,handleSubmit,getValues,formState:{errors,isSubmitted,isSubmitting}} = useForm({mode: "onChange"})

    const join = handleSubmit((data)=>{
        console.log(data);
        const {memberEmail,memberPassword} = data;
        // fetch("http://localhost:3001/posts",{
        //     method: "POST",
        //     headers: {
        //         "Content-Type": "application/json"
        //     },
        //     body: JSON.stringify({
        //         memberEmail:memberEmail,
        //         memberPassword:memberPassword
        //     })
        // })
        // fetch("http://localhost:3001/posts/27e2",{
        //     method: "PUT",
        //     headers: {
        //         "Content-Type": "application/json"
        //     },
        //     body: JSON.stringify({
        //         memberEmail:memberEmail,
        //         memberPassword:memberPassword
        //     })
        // })
        fetch("http://localhost:3001/posts/27e2",{
            method: "DELETE"
        })
    })

    const emailRegex = /^[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}$/;
    const passwordRegex = /^(?=.*[A-Za-z])(?=.*\d)(?=.*[!@#$%^&*()_+[\]{};':"\\|,.<>/?]).{8,}$/;

    return (
        <div>
            <form onSubmit={join}>
                <label>
                    <p>이메일</p>
                    <input 
                        type="text"
                        placeholder='이메일을 입력하세요.'
                        {...register("memberEmail",{
                            required: true,
                            pattern: emailRegex
                        })}
                    />
                    {errors && errors?.memberEmail?.type === "required" && <S.ErrorMessage>이메일은 필수 입력입니다.</S.ErrorMessage>}
                    {errors && errors?.memberEmail?.type === "pattern" && <S.ErrorMessage>이메일 형식이 아닙니다.</S.ErrorMessage>}
                </label>

                <label>
                    <p>비밀번호</p>
                    <input 
                        type="password"
                        placeholder='비밀번호를 입력하세요.'
                        {...register("memberPassword",{
                            required: true,
                            pattern: passwordRegex
                        })}
                    />
                    {errors && errors?.memberPassword?.type === "required" && <S.ErrorMessage>비밀번호는 필수 입력입니다.</S.ErrorMessage>}
                    {errors && errors?.memberPassword?.type === "pattern" && <S.ErrorMessage>비밀번호 형식이 아닙니다.</S.ErrorMessage>}
                </label>
                <label>
                    <p>비밀번호 확인</p>
                    <input 
                        type="password"
                        placeholder='비밀번호를 입력하세요.'
                        {...register("memberPasswordConfirm",{
                            required: true,
                            validate:{
                                matchPassword:(memberPasswordConfirm)=> {
                                    const {memberPassword} = getValues();
                                    return memberPasswordConfirm === memberPassword;
                                }
                            }
                        })}
                    />
                    {errors && errors?.memberPasswordConfirm && (
                        errors?.memberPasswordConfirm && <S.ErrorMessage>비밀번호가 일치하지 않습니다.</S.ErrorMessage>
                    )}
                </label>

                {/* 체크박스 */}
                <div>
                    <p>취미</p>
                    <label><input type="checkbox" name="hobby" value="reading" {...register("hobbies")} /><span>독서</span></label>
                    <label><input type="checkbox" name="hobby" value="music" {...register("hobbies")} /><span>음악</span></label>
                    <label><input type="checkbox" name="hobby" value="sports" {...register("hobbies")} /><span>운동</span></label>
                </div>
                <button disabled={isSubmitting}>회원가입</button>
            </form>
        </div>
    );
};

export default Join;